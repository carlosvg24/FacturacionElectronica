...
namespace Facturacion.DAL
{
	public class DALFormapagoCat: ENTFormapagoCat
	{
		...
		public void Agregar()
		{
			...
			SqlParameter param7 = new SqlParameter("@EsFacturablePortalAdmin", SqlDbType.Bit);
			param7.Value = EsFacturablePortalAdmin;
			commandParameters.Add(param7);
			SqlParameter param8 = new SqlParameter("@EsFacturableGlobal", SqlDbType.Bit);
			param8.Value = EsFacturableGlobal;
			...
			try
			{
				...
				foreach (DataRow dr in dtResultado.Rows)
				{
				    ENTFormapagoCat item = new ENTFormapagoCat();
					 ...
					 if (!dr.IsNull("EsFacturablePortalAdmin")) item.EsFacturablePortalAdmin = Convert.ToBoolean(dr["EsFacturablePortalAdmin"]);
		...
		public List<ENTFormapagoCat> RecuperarFormapagoCatPorLlavePrimaria(int idformapago)
		{
			...
			try
			{
				...
				if (dtResultado.Rows.Count.Equals(1))
				{
					...
					if (!dtResultado.Rows[0].IsNull("EsFacturablePortalAdmin")) EsFacturablePortalAdmin = Convert.ToBoolean(dtResultado.Rows[0]["EsFacturablePortalAdmin"]);
