...
namespace Facturacion.DAL
{
	public class DALReservaCab: ENTReservaCab
	{
		...
		public void Agregar()
		{
            ...
            SqlParameter param19 = new SqlParameter("@EmailAddress", SqlDbType.VarChar);
            param19.Value = EmailAddress==null?String.Empty:EmailAddress;
        ...
        #region Actualizar Email de Reservación
        public void ActualizarEmail()
        {
            ...
            SqlParameter param1 = new SqlParameter("@RecordLocator", SqlDbType.VarChar);
            param1.Value = RecordLocator;
            commandParameters.Add(param1);
            SqlParameter param2 = new SqlParameter("@EmailAddress", SqlDbType.VarChar);
            param2.Value = EmailAddress;
            commandParameters.Add(param2);
            cmm.CommandType = CommandType.StoredProcedure;
            cmm.CommandText = "uspFac_UpdEmailReservaCab";
            cmm.CommandTimeout = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["SqlCommandTimeOut"]);
            cmm.Connection = _conexion;
            bool cerrarConexion = false;
            foreach (SqlParameter p in commandParameters)
			...
			try
			{
				...
				foreach (DataRow dr in dtResultado.Rows)
				{
				    ENTReservaCab item = new ENTReservaCab();
					 ...
					 //if (!dr.IsNull("EmailAddress")) item.EmailAddress = dr["EmailAddress"].ToString();
				    result.Add(item);
				}
			}
			catch(SqlException ex)
		...
		public List<ENTReservaCab> RecuperarReservaCabPorLlavePrimaria(long idreservacab)
		{
			...
			try
			{
				...
				if (dtResultado.Rows.Count.Equals(1))
				{
					...
					//if (!dtResultado.Rows[0].IsNull("EmailAddress")) EmailAddress = dtResultado.Rows[0]["EmailAddress"].ToString();
					this.Clone();
				}
			}
			catch(SqlException ex)
        ...
        #region Recuperar Reservaciones sin Facturar y sin e-mail
        public List<String> RecuperarReservaSinFacturarNiEmail()
        {
            ...
            SqlCommand cmm = new SqlCommand();
            cmm.CommandType = CommandType.StoredProcedure;
            cmm.CommandText = "uspFac_GetReservaCab_SinFacturarNiEmail";
            cmm.CommandTimeout = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["SqlCommandTimeOut"]);
            cmm.Connection = _conexion;
            bool cerrarConexion = false;
            ...
            try
            {
                ...
                foreach (DataRow dr in dtResultado.Rows)
                {
                    result.Add(dr["RecordLocator"].ToString());
                }
            }
            catch (SqlException ex)
            {
        ...
        #region Recuperar Reservaciones pendientes de facturar por eMail
        public List<String> RecuperarReservaPendFacPorEmail(string email)
        {
            List<String> result = new List<String>();
            DataTable dtResultado = new DataTable();
            List<SqlParameter> commandParameters = new List<SqlParameter>();
            SqlParameter param0 = new SqlParameter("@EmailAddress", SqlDbType.VarChar);
            param0.Value = email;
            commandParameters.Add(param0);
            SqlCommand cmm = new SqlCommand();
            cmm.CommandType = CommandType.StoredProcedure;
            cmm.CommandText = "uspFac_GetReservaPendFac_POR_Email";
            cmm.CommandTimeout = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["SqlCommandTimeOut"]);
            cmm.Connection = _conexion;
            bool cerrarConexion = false;
            foreach (SqlParameter p in commandParameters)
